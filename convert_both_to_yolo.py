...
def convert_labelme_to_yolo(json_file, output_dir):
    ...
    for shape in data['shapes']:
        if shape['label'] == 'outline':
            ...
            # Add both outlined (class 0) and solid (class 1) pieces
            # For solid pieces, slightly adjust the position to avoid perfect overlap
            offset = 0.01  # 1% of image size
            x_center_solid = x_center + random.uniform(-offset, offset)
            y_center_solid = y_center + random.uniform(-offset, offset)
            
            # Add outlined piece (class 0)
            yolo_labels.append(f"0 {x_center:.6f} {y_center:.6f} {w:.6f} {h:.6f}")
            # Add solid piece (class 1)
            yolo_labels.append(f"1 {x_center_solid:.6f} {y_center_solid:.6f} {w:.6f} {h:.6f}")
